    ...
    RxDma: embassy_stm32::usart::RxDma<T>,
{
	...
	let mut nb_to_read:u8=0;
	    ...
	    else if (rx_state == RxState::RxReadingHeader3) //Here we should have read the complete header 0xff 0xff id length
	    {
	    ...
	    else if (rx_state == RxState::RxMessageReady) //should not happen
	    {
		return Err(CommunicationError::DynamixelParsingError(ParsingError::InvalidPacket));
	    }
	...
	let mut nb_to_read:usize=1;
	    ...
	    let ret = match self.usart.read(&mut self.read_buffer[buf_idx..total+nb_to_read]).await {
	    ...
	    else if (rx_state == RxState::RxReadingHeader2) //id
	    {
