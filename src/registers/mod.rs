...
macro_rules! define_register_map {
    ($map_struct_name:ident, $enum_name:ident, $buffer:ident, $mutex_type:ty, $word_size:expr, $($name:ident, $address:expr, $byte_size:expr, $access_type:expr),+ $(,)?) => {
	    ...
	    pub fn [<$map_struct_name _get_size>](register: $enum_name) -> usize {
		let reg = &$map_struct_name[register as usize];
		reg.byte_size.into()
	    }


	    pub async fn [<$map_struct_name _read_by_name>](register: $enum_name, buffer: &mut [u8]) -> Result<usize, ()> {
		let reg = &$map_struct_name[register as usize];
		...
		Ok(end - start)
            ...
            pub async fn [<$map_struct_name _write_by_name>](register: $enum_name, value: &[u8]) -> Result<(), ()> {
                let reg = &$map_struct_name[register as usize];
	    ...
	    pub async fn [<$map_struct_name _read_by_address>](
            ...
            pub async fn [<$map_struct_name _write_by_address>](
