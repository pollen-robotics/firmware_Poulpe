...
pub async fn main(_spawner: Spawner) {
    ...
    match controller.set_control_mode(MotionMode::Torque){
    ...
    led_green.set_high();
    loop {
        controller.set_target_torque([500.0]);

        let current_torque = controller.get_current_torque().unwrap_or_default()[0];
        ticker.next().await;
        info!(
