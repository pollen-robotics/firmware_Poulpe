...
pub async fn control_loop(mut config: ActuatorConfig) {
    ...
    let emergency_stop_response_counter_max = emergency_stop_torque_limits.len()
        * ((emergency_stop_time_per_torque_secs * 1000.0) as usize); // 3secs per torque limit (1000 loops at 1kHz)
